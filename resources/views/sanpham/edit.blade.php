    ...
    $(document).ready(function() {
        ...
        $("#sp_hinhanhlienquan").fileinput({
            ...
            initialPreview: [
                ...
                "{{ asset('storage/photos/' . $hinhAnh->hsp_ten) }}",
                @endforeach
            ],
            initialPreviewConfig: [
                @foreach($sp->hinhanhlienquan()->get() as $index=>$hinhAnh)
                {
                    caption: "{{ $hinhAnh->hsp_ten }}", 
                    size: {{ Storage::exists('public/photos/' . $hinhAnh->hsp_ten) ? Storage::size('public/photos/' . $hinhAnh->hsp_ten) : 0 }}, 
                    width: "120px", 
                    url: "{$url}", 
                    key: {{ ($index + 1) }}
                },
